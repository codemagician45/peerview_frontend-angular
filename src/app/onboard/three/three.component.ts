...
export class ThreeComponent implements OnInit {
    ...
    toggleInterest(interest: any) {
      ...
      this.selectedInterests.forEach((item) =>{
        if(this.selectedInterests.length > 0){
              const catId = item.id;
              this._courseService.getSubInterest(catId).subscribe((resp) => {
              this.subinterests = resp["interests"];
   ...
   //      console.log(this.subinterests,"xcfvdfdsf")
    ...
    toggleSubInterest(subinterest) {
        console.log(this.subinterests,subinterest)
        const sI = _.findIndex(this.subinterests, subinterest);
           console.log(sI,"sI")
        let interest = this.interests.filter(interest => {
            console.log(this.subinterests[sI].id,sI,"sI")
            return interest.id == this.subinterests[sI].id;
        })[0];
        ...
        this.subinterests[sI].isselected = !this.subinterests[sI].isselected;

        this.subinterests.forEach(
    ...
    submitInterests() {
        const finalInterests =  _.filter(this.subinterests, (item) => {
